...
//		int rows = noteDao.updateNote(note);//更新
	...
	String userId, String noteTitle, String bookId) {
		...
		testNote.setCn_user_id(userId);
		testNote.setCn_note_title(noteTitle);
		testNote.setCn_notebook_id(bookId);
		testNote = noteDao.findByNoteName(testNote);
		if(testNote!=null){
			result.setStatus(1);
			result.setMsg("笔记命名重复，请重新命名");
			return result;
		}
		//创建note参数保存
		Note note = new Note();
...
//			noteDao.updateBookId(note);//更新
	...
	public NoteResult shareNote(String noteId) {
		...
		if("2".equals(note.getCn_note_type_id())){
			//删除分享中的笔记
			shareDao.removeByNoteId(noteId);
			//更新笔记的cn_note_type_id为分享状态'1'
